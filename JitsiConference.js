...
export default class JitsiConference {
    ...
    isEndConferenceSupported() {
        return Boolean(
            this.room && this.room.xmpp.endConferenceComponentAddress
        );
    }
    ...
    _removeLocalSourceOnReject(jingleSession, error, mediaType) {
        ...
        logger.warn(
            `Source-add rejected on ${jingleSession}, reason="${error?.reason}", message="${error?.msg}"`
        );
    ...
    _setNewVideoType(track) {
        ...
        if (track) {
            videoTypeChanged = this._signalingLayer.setTrackVideoType(
                track.getSourceName(),
                track.videoType
            );
        }
    ...
    setDesktopSharingFrameRate(maxFps) {
        ...
        this.jvbJingleSession &&
            this.jvbJingleSession.peerconnection.setDesktopSharingFrameRate(
                fps
            );
