...
class ScreenObtainer {
    ...
    private _getAudioConstraints(): boolean | IAudioQuality {
        ...
        let audio: boolean | IAudioQuality =
            audioQuality?.stereo || needsEchoCancellation
        ...
        if (
            ...
            window.JitsiMeetScreenObtainer.openDesktopPicker(
                ...
                (
                    ...
                    if (streamId) {
                        let audioConstraints: boolean | IAudioConstraints =
                            false;
                        ...
                        if (screenShareAudio) {
                            ...
                            if (streamType === "screen") {
                                (
                                    audioConstraints as IAudioConstraints
                                ).mandatory = {
    ...
    private _obtainScreenFromGetDisplayMedia(
        callback: (result: IScreenCaptureResult) => void,
        errorCallback: (error: JitsiTrackError) => void,
        options: IObtainScreenOptions = {}
    ) {
        let getDisplayMedia;
     ...
     * @internal
     */
    obtainScreenFromGetDisplayMediaRN(
        callback: (result: IScreenCaptureResult) => void,
        errorCallback: (error: JitsiTrackError) => void
    ) {
