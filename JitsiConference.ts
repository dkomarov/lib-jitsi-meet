...
export default class JitsiConference extends Listenable {
    ...
    authenticateAndUpgradeRole(options) {
        // TODO: fix types after migration of authenticateAndUpgradeRole.js
    ...
    _sendBridgeVideoTypeMessage(localtrack: JitsiLocalTrack): void {
        ...
        if (
            videoType === BridgeVideoType.DESKTOP &&
        ...
        if (
            ...
            this._sessionInitiateTimeout = window.setTimeout(() => {
                ...
                Statistics.sendAnalytics(
                    createJingleEvent(ACTION_JINGLE_SI_TIMEOUT, {
                        p2p: false,
    ...
    _acceptJvbIncomingCall(
        ...
        this._maybeClearSITimeout();
        Statistics.sendAnalytics(
            createJingleEvent(ACTION_JINGLE_SI_RECEIVED, {
                p2p: false,
                value: now,
            })
        );
    ...
    _setBridgeChannel(offerIq: object, pc: TraceablePeerConnection): void {
        ...
        const sctpOffered =
            $(offerIq).find('>content[name="data"]').first().length === 1;
        let wsUrl = null;
    ...
    getSsrcByTrack(track: JitsiTrack): Optional<number> {
        return track.isLocal()
            ? this.getActivePeerConnection()?.getLocalSSRC(track)
            : track.getSsrc();
    }
    ...
    _onIceConnectionFailed(session: JingleSessionPC): void {
        if (session.isP2P) {
            ...
            if (this.p2pJingleSession) {
                Statistics.sendAnalyticsAndLog(
                    createP2PEvent(ACTION_P2P_FAILED, {
            ...
            Statistics.sendAnalyticsAndLog(
                createJvbIceFailedEvent(ACTION_JVB_ICE_FAILED, {
    ...
    _onIceConnectionEstablished(jingleSession: JingleSessionPC): void {
        ...
        if (
            ...
            Statistics.sendAnalytics(ICE_ESTABLISHMENT_DURATION_DIFF, {
        ...
        Statistics.sendAnalyticsAndLog(
            createP2PEvent(ACTION_P2P_ESTABLISHED, {
                initiator: this.p2pJingleSession.isInitiator,
            })
        );
    }
