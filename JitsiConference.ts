...
import VADAudioAnalyser, {
    IVADProcessor,
} from "./modules/detection/VADAudioAnalyser";
...
import RecordingManager, {
    IRecordingOptions,
} from "./modules/recording/RecordingManager";
...
function _getCodecMimeType(codec: string): Nullable<CodecMimeType> {
    if (typeof codec === "string") {
        return (
            Object.values(CodecMimeType).find(
...
export default class JitsiConference extends Listenable {
    ...
    constructor(options: IConferenceOptions) {
        ...
        if (
            !options.name ||
            options.name.toLowerCase() !== options.name.toString()
        ) {
            const errmsg =
            ...
            const additionalLogMsg = options.name
                ? `roomName=${
                      options.name
                  }; condition - ${options.name.toLowerCase()}!==${options.name.toString()}`
                : "No room name passed!";
         ...
         */
        this.avgRtpStatsReporter = new AvgRTPStatsReporter(
            this,
            options.config.avgRtpStatsN || 15
        );
        ...
        const delay = Number.parseInt(
            String(options.config.p2p?.backToP2PDelay || 5),
            10
        );
    ...
    private _init(options: IConferenceOptions): void {
        ...
        this._statsCurrentId =
            config.statisticsId ?? Settings.callStatsUserName;
        ...
        this.room.addListener(
            XMPPEvents.CONNECTION_ICE_FAILED,
            this._onIceConnectionFailed
        );
        ...
        this._onIceConnectionRestored =
            this._onIceConnectionRestored.bind(this);
        this.room.addListener(
            XMPPEvents.CONNECTION_RESTORED,
            this._onIceConnectionRestored
        );

        this._onP2PTerminationRequired =
            this._onP2PTerminationRequired.bind(this);
        this.room.addListener(
            XMPPEvents.P2P_TERMINATION_REQUIRED,
            this._onP2PTerminationRequired
        );
        ...
        this._sendConferenceJoinAnalyticsEvent =
            this._sendConferenceJoinAnalyticsEvent.bind(this);
        this.room.addListener(
            XMPPEvents.MEETING_ID_SET,
            this._sendConferenceJoinAnalyticsEvent
        );

        this._removeLocalSourceOnReject =
            this._removeLocalSourceOnReject.bind(this);
        this._updateRoomPresence = this._updateRoomPresence.bind(this);
        this.room.addListener(
            XMPPEvents.SESSION_ACCEPT,
            this._updateRoomPresence
        );
        ...
        const qualityOptions = {
            ...
            jvb: {
                ...
                preferredCodec: _getCodecMimeType(
                    config.videoQuality?.preferredCodec
                ),
                screenshareCodec: browser.isMobileDevice()
                    ? _getCodecMimeType(
                          config.videoQuality?.mobileScreenshareCodec
                      )
        ...
        if (!this.statistics) {
            this.statistics = new Statistics(this, {
                ...
                confID:
                    config.confID ??
                    `${this.connection.options.hosts.domain}/${this.options.name}`,
        ...
        if (config.enableTalkWhileMuted && browser.supportsVADDetection()) {
            ...
            if (config.createVADProcessor) {
                ...
                if (!this._audioAnalyser) {
                    this._audioAnalyser = new VADAudioAnalyser(
                        this,
                        config.createVADProcessor
                    );
                }

                const vadTalkMutedDetection = new VADTalkMutedDetection();

                vadTalkMutedDetection.on(
                    DetectionEvents.VAD_TALK_WHILE_MUTED,
                    () =>
                        this.eventEmitter.emit(
                            JitsiConferenceEvents.TALK_WHILE_MUTED
                        )
                );
                this._audioAnalyser.addVADDetectionService(
                    vadTalkMutedDetection
                );
            } else {
                logger.warn(
                    "No VAD Processor was provided. Talk while muted detection service was not initialized!"
                );
            }
        }
        ...
        if (config.enableNoisyMicDetection && browser.supportsVADDetection()) {
            if (config.createVADProcessor) {
                ...
                logger.warn(
                    "No VAD Processor was provided. Noisy microphone detection service was not initialized!"
                );
            }
        }
        ...
        if (
            config.enableNoAudioDetection &&
            !config.disableAudioLevels &&
            LocalStatsCollector.isLocalStatsSupported()
        ) {
            this._noAudioSignalDetection = new NoAudioSignalDetection(this);
            this._noAudioSignalDetection.on(
                DetectionEvents.NO_AUDIO_INPUT,
                () =>
            ...
            this._noAudioSignalDetection.on(
                ...
                (hasAudioSignal) =>
                    this.eventEmitter.emit(
                        JitsiConferenceEvents.AUDIO_INPUT_STATE_CHANGE,
                        hasAudioSignal
                    )
            );
        }
        ...
        this.p2pDominantSpeakerDetection = new P2PDominantSpeakerDetection(
            this
        );
        ...
        const transcriptionLanguage =
            config?.transcriptionLanguage ?? DEFAULT_TRANSCRIPTION_LANGUAGE;
        ...
        rtc.addListener(RTCEvents.DATA_CHANNEL_OPEN, () => {
            for (const localTrack of this.rtc.localTracks) {
                localTrack.isVideoTrack() &&
                    this._sendBridgeVideoTypeMessage(localTrack);
            }
        });
    }
    ...
    private _sendConferenceJoinAnalyticsEvent(): void {
        ...
        if (
            this._conferenceJoinAnalyticsEventSent ||
            !meetingId ||
            this.getActivePeerConnection() === null
        ) {
            return;
        }
    ...
    private _sendConferenceLeftAnalyticsEvent(): void {
        ...
        Statistics.sendAnalytics(
            createConferenceEvent("left", {
                meetingId,
                participantId: `${meetingId}.${this._statsCurrentId}`,
                stats: {
                    duration: Math.floor(
                        (Date.now() - this._conferenceJoinAnalyticsEventSent) /
                            1000
                    ),
                },
            })
    ...
    private _fireMuteChangeEvent(track: JitsiLocalTrack): void {
        ...
        if (this.isMutedByFocus && track.isAudioTrack() && !track.isMuted()) {
            ...
            this.room.muteParticipant(
                this.room.myroomjid,
                false,
                MediaType.AUDIO
            );
        } else if (
            this.isVideoMutedByFocus &&
            track.isVideoTrack() &&
        ...
        if (this.mutedByFocusActor && track.isAudioTrack()) {
            ...
            this.mutedVideoByFocusActor &&
            track.isVideoTrack() &&
            track.getVideoType() !== VideoType.DESKTOP
        ) {
            const actorId = Strophe.getResourceFromJid(
                this.mutedVideoByFocusActor
            );

            actorParticipant = this.participants.get(actorId);
        } else if (
            this.mutedDesktopByFocusActor &&
            track.isVideoTrack() &&
            track.getVideoType() === VideoType.DESKTOP
        ) {
            const actorId = Strophe.getResourceFromJid(
    ...
    private _removeLocalSourceOnReject(
        ...
        logger.warn(
            `Source-add rejected on ${jingleSession}, reason="${errorReason}", message="${error?.message}"` +
        ...
        for (const track of rejectedTracks) {
            this.eventEmitter.emit(
                JitsiConferenceEvents.TRACK_UNMUTE_REJECTED,
                track
            );
        }
    }
    ...
    private _setupNewTrack(newTrack: JitsiLocalTrack): void {
        ...
        this._unsubscribers.push(
            newTrack.addCancellableListener(
                JitsiTrackEvents.TRACK_MUTE_CHANGED,
                this._fireMuteChangeEvent.bind(this, newTrack)
            )
        );
        ...
        if (newTrack.isAudioTrack()) {
            this._unsubscribers.push(
                newTrack.addCancellableListener(
                    ...
                    this._fireAudioLevelChangeEvent.bind(this)
                )
            );
        }
    ...
    private _removeRemoteJVBTracks(): void {
        this._removeRemoteTracks(
            "JVB",
            this.jvbJingleSession.peerconnection.getRemoteTracks()
        );
    }
    ...
    private _removeRemoteP2PTracks(): void {
        this._removeRemoteTracks(
            "P2P",
            this.p2pJingleSession.peerconnection.getRemoteTracks()
        );
    }
    ...
    private _setP2PStatus(newStatus: boolean): void {
        ...
        if (newStatus) {
            ...
            this.p2pJingleSession
                ...
                .catch((error) => {
                    logger.error(
                        `Failed to sync up P2P video transfer status (${isVideoActive}), ${error}`
                    );
                });
        } else {
    ...
    private _startP2PSession(remoteJid: string): void {
        ...
        this.isP2PConnectionInterrupted = false;
        this.p2pJingleSession = this.xmpp.connection.jingle.newP2PJingleSession(
            this.room.myroomjid,
            remoteJid
        );
        logger.info(
            "Created new P2P JingleSession",
            this.room.myroomjid,
            remoteJid
        );
        this._sendConferenceJoinAnalyticsEvent();
        ...
        this.p2pJingleSession.initialize(
            ...
            this._signalingLayer,
            {
                ...
                codecSettings: {
                    codecList:
                        this.qualityController.codecController.getCodecPreferenceList(
                            "p2p"
                        ),
                    mediaType: MediaType.VIDEO,
                    screenshareCodec:
                        this.qualityController.codecController.getScreenshareCodec(
                            "p2p"
                        ),
                },
                enableInsertableStreams:
                    this.isE2EEEnabled() ||
                    FeatureFlags.isRunInLiteModeEnabled(),
            }
        );
            ...
            .catch((error) => {
                ...
                if (this.p2pJingleSession) {
                    this.eventEmitter.emit(
                        ...
                        JitsiConferenceErrors.OFFER_ANSWER_FAILED,
                        error
                    );
                }
            });
    ...
    private _suspendMediaTransferForJvbConnection(): void {
        ...
        this.jvbJingleSession
            .setMediaTransferActive(false)
            .then(() => {
                logger.info(
                    "Suspended media transfer over the JVB connection !"
                );
            })
            .catch((error) => {
                logger.error(
    ...
    private _maybeStartOrStopP2P(userLeftEvent: boolean = false): void {
        ...
        if (!this.p2pJingleSession && shouldBeInP2P) {
            ...
            if (this.options.config.testing?.forceInitiator) {
                logger.debug(
                    `Forcing P2P initiator, will start P2P with: ${jid}`
                );
                this._startP2PSession(jid);
            } else if (this.options.config.testing?.forceResponder) {
                logger.debug(
                    `Forcing P2P responder, will wait for the other peer ${jid} to start P2P`
                );
            } else {
                if (myId > peersId) {
                    logger.debug(
                        "I'm the bigger peersId - the other peer should start P2P",
                        myId,
                        peersId
                    );
                ...
                if (userLeftEvent) {
                    ...
                    logger.info(
                        `Will start P2P with: ${jid} after ${this.backToP2PDelay} seconds...`
                    );
                    this.deferredStartP2PTask = Number(
                        setTimeout(
            ...
            logger.info(
                `Will stop P2P with: ${this.p2pJingleSession.remoteJid}`
            );
    ...
    private _stopP2PSession(
        ...
        this.p2pJingleSession.terminate(
            ...
            (error) => {
                ...
                if (reason !== "success") {
                    logger.error(
                        "An error occurred while trying to terminate P2P Jingle session",
                        error
                    );
                }
            },
                ...
                sendSessionTerminate: Boolean(
                    this.room &&
                        this.getParticipantById(
                            Strophe.getResourceFromJid(
                                this.p2pJingleSession.remoteJid
                            )
                        )
                ),
            }
    ...
    private _updateRoomPresence(
        ...
        for (const track of localTracks) {
            ...
            muteStatusChanged &&
                logger.debug(
                    `Updating mute state of ${track} in presence to muted=${muted}`
                );
            if (track.getType() === MediaType.VIDEO) {
                videoTypeChanged = this._setNewVideoType(track);
                videoTypeChanged &&
                    logger.debug(
                        `Updating videoType in presence to ${track.getVideoType()}`
                    );
            }
            presenceChanged =
    ...
    private _updateFeatures(participant: JitsiParticipant): void {
        participant
            .getFeatures()
            .then((features) => {
                participant._supportsDTMF = features.has(
    ...
    private _acceptJvbIncomingCall(
        ...
        if (this.wasStopped) {
            Statistics.sendAnalyticsAndLog(
                createJingleEvent(AnalyticsEvents.ACTION_JINGLE_RESTART, {
                    p2p: false,
                })
            );
        }
        ...
        const serverRegion = getAttribute(
            findFirst(
                jingleOffer,
                ':scope>bridge-session[*|xmlns="http://jitsi.org/protocol/focus"]'
            ),
            "region"
        );
        ...
        Statistics.sendAnalytics(
            createJingleEvent(AnalyticsEvents.ACTION_JINGLE_SI_RECEIVED, {
                p2p: false,
                value: now,
            })
        );
        ...
        try {
            jingleSession.acceptOffer(
                ...
                (error) => {
                    logger.error(
                        "Failed to accept incoming JVB Jingle session",
                        error
                    );
                    this.eventEmitter.emit(
                        JitsiConferenceEvents.CONFERENCE_FAILED,
            ...
            this._desktopSharingFrameRate &&
                jingleSession.peerconnection.setDesktopSharingFrameRate(
                    this._desktopSharingFrameRate
                );
    ...
    private _setBridgeChannel(
        ...
        const ignoreDomain =
            this.connection?.options?.bridgeChannel?.ignoreDomain;
        const preferSctp =
            this.connection?.options?.bridgeChannel?.preferSctp ?? true;
        const sctpOffered =
            findAll(offerIq, ':scope>content[name="data"]').length === 1;
        let wsUrl = null;
            ...
            findAll(offerIq, ":scope>content>transport>web-socket")
                ...
                .forEach((url) => {
                    if (
                        !wsUrl &&
                        (!ignoreDomain ||
                            ignoreDomain !== new URL(url).hostname)
                    ) {
                        wsUrl = url;
                        logger.info(`Using colibri-ws url ${url}`);
                    } else if (!wsUrl) {
                        logger.info(
                            `Ignoring colibri-ws url with domain ${ignoreDomain}`
                        );
                    }
                });

            if (!wsUrl) {
                const firstWsUrl = findFirst(
                    offerIq,
                    ":scope>content>transport>web-socket"
                );
        ...
        if (wsUrl && !(sctpOffered && preferSctp)) {
            ...
            logger.warn(
                "Neither SCTP nor a websocket is available. Will not initialize bridge channel."
            );
        }
    }
    ...
    private _onIncomingCallP2P(
        ...
        let rejectReason;
        const contentName = getAttribute(
            findFirst(jingleOffer, ":scope>content"),
            "name"
        );
        ...
        if (!peerUsesUnifiedPlan) {
            ...
            rejectReason = {
                errorMsg:
                    "P2P mode disabled in the configuration or browser unsupported",
                reason: "decline",
                reasonDescription: "P2P disabled",
            };
            ...
            Statistics.sendAnalytics(
                createJingleEvent(AnalyticsEvents.ACTION_P2P_DECLINED)
            );
        }
    ...
    private _onIceConnectionFailed(session: JingleSessionPC): void {
        if (session.isP2P) {
            ...
            if (this.p2pJingleSession) {
                Statistics.sendAnalyticsAndLog(
                    createP2PEvent(AnalyticsEvents.ACTION_P2P_FAILED, {
            ...
            this._iceRestarts < MAX_CONNECTION_RETRIES
        ) {
            ...
            Statistics.sendAnalyticsAndLog(
                createJvbIceFailedEvent(AnalyticsEvents.ACTION_JVB_ICE_FAILED, {
    ...
    private _onP2PTerminationRequired(session: JingleSessionPC): void {
        ...
        logger.warn(
            "Stopping P2P session due to SSRC regeneration on P2P connection"
        );
        this._stopP2PSession({
            reason: "connectivity-error",
    ...
    private _onIceConnectionEstablished(jingleSession: JingleSessionPC): void {
        ...
        if (!jingleSession.isP2P) {
            ...
            logger.error(
                "CONNECTION_ESTABLISHED - wrong P2P session instance ?!"
            );
        ...
        if (
            isValidNumber(this.p2pEstablishmentDuration) &&
        ...
        Statistics.sendAnalyticsAndLog(
            createP2PEvent(AnalyticsEvents.ACTION_P2P_ESTABLISHED, {
                initiator: this.p2pJingleSession.isInitiator,
            })
        );
    }
    ...
    private _updateProperties(properties: IConferenceProperties = {}): void {
        ...
        if (changed) {
            ...
            const audioLimitReached =
                this.properties["audio-limit-reached"] === "true";
            const videoLimitReached =
                this.properties["video-limit-reached"] === "true";
            ...
            if (this._audioSenderLimitReached !== audioLimitReached) {
                ...
                logger.info(
                    `Audio unmute permissions set by Jicofo to ${audioLimitReached}`
                );
            }
            ...
            if (this._videoSenderLimitReached !== videoLimitReached) {
                ...
                logger.info(
                    `Video unmute permissions set by Jicofo to ${videoLimitReached}`
                );
            }
            ...
            if (publishedCodecs?.length) {
                publishedCodecs = publishedCodecs.filter(
                    (codec) =>
                        typeof codec === "string" &&
                        codec.trim().length &&
                        Object.values(CodecMimeType).find(
    ...
    _sendBridgeVideoTypeMessage(localtrack: JitsiLocalTrack): void {
        let videoType =
            !localtrack || localtrack.isMuted()
                ? BridgeVideoType.NONE
                : localtrack.getVideoType();

        if (
            videoType === VideoType.DESKTOP &&
            this._desktopSharingFrameRate > SS_DEFAULT_FRAME_RATE
        ) {
    ...
    _stopJvbSession(options?: {
        ...
        const { requestRestart = false, sendSessionTerminate = false } =
            options;
        ...
        this.jvbJingleSession.terminate(
            ...
            (error) => {
                if (requestRestart && sendSessionTerminate) {
                    logger.error(
                        "session-terminate for ice restart failed: reloading the client"
                    );
                ...
                logger.error(
                    `An error occurred while trying to terminate the JVB session', reason=${error.reason},` +
                        `msg=${error.msg}`
                );
            },
            options
    ...
    _removeLocalTrackFromPc(
        ...
        if (track.conference === this) {
            if (this.jvbJingleSession) {
                removePromises.push(
                    this.jvbJingleSession.removeTrackFromPc(track)
                );
            } else {
                logger.debug(
            ...
            if (this.p2pJingleSession) {
                removePromises.push(
                    this.p2pJingleSession.removeTrackFromPc(track)
                );
            } else {
                logger.debug(
    ...
    async _addLocalTrackToPc(track: JitsiLocalTrack): Promise<void> {
        ...
        if (track.conference === this) {
            if (this.jvbJingleSession) {
                ...
                logger.debug(
                    "Add local MediaStream - no JVB Jingle session started yet"
                );
            }
            ...
            if (this.p2pJingleSession) {
                ...
                logger.debug(
                    "Add local MediaStream - no P2P Jingle session started yet"
                );
            }
        } else {
    ...
    _setTrackMuteStatus(
        ...
        if (localTrack) {
            ...
            presenceChanged &&
                logger.debug(
                    `Mute state of ${localTrack} changed to muted=${isMuted}`
                );
        }
                    ...
                    room.xmpp.moderator
                        ...
                        .then(() => {
                            ...
                            this.room.xmpp.moderator
                                ...
                                .catch((e) =>
                                    logger.trace(
                                        "sendConferenceRequest rejected",
                                        e
                                    )
                                )
                                .finally(() => {
                                    ...
                                    this.room.xmpp.moderator.conferenceRequestSent =
                                        false;
   ...
   */
    ...
    public sendCommand(name: string, values: Record<string, unknown>): void {
        if (this.room) {
            this.room.addOrReplaceInPresence(name, values) &&
    ...
    public setSubject(subject: string): void {
        if (this.room && this.isModerator()) {
            ...
            logger.warn(
                `Failed to set subject, ${this.room ? "" : "not in a room, "}${
    ...
    public addTrack(track: JitsiLocalTrack): Promise<void> {
        ...
        if (localTracks.length > 0) {
            ...
            if (
                ...
                (mediaType === MediaType.VIDEO &&
                    !localTracks.find(
                        (t) => t.getVideoType() === track.getVideoType()
                    ))
            ) {
                const sourceName = getSourceNameForJitsiTrack(
                    this.myUserId(),
                ...
                this.p2pJingleSession &&
                    addTrackPromises.push(
                        this.p2pJingleSession.addTracks([track])
                    );
                this.jvbJingleSession &&
                    addTrackPromises.push(
                        this.jvbJingleSession.addTracks([track])
                    );
            ...
            return Promise.reject(
                new Error(
                    `Cannot add second ${mediaType} track to the conference`
                )
            );
        }
    ...
    public replaceTrack(
        ...
        if (oldTrack && !oldTrackBelongsToConference) {
            logger.warn(
                `JitsiConference.replaceTrack oldTrack (${oldTrack} does not belong to this conference`
            );
        }

        // Now replace the stream at the lower levels
        return this._doReplaceTrack(
            oldTrackBelongsToConference ? oldTrack : null,
            newTrack
        )
            .then(() => {
                if (
                    oldTrackBelongsToConference &&
                    !oldTrack.isMuted() &&
                    !newTrack
                ) {
                ...
                if (
                    (oldTrackBelongsToConference && oldTrack?.isVideoTrack()) ||
    ...
    public isHidden(): boolean {
        ...
        return (
            Strophe.getDomainFromJid(this.connection.getJid()) ===
    ...
    onMemberJoined(
        ...
        if (
            (this.jvbJingleSession &&
                this.getActiveMediaSession() === this.jvbJingleSession) ||
            ((typeof startAudioMuted === "undefined" ||
                startAudioMuted === -1) &&
                (typeof startVideoMuted === "undefined" ||
                    startVideoMuted === -1))
        ) {
            return;
        }
        ...
        if (
            (audioMuted && !this.startMutedPolicy.audio) ||
            (videoMuted && !this.startMutedPolicy.video)
        ) {
    ...
    onMemberLeft(jid: string, reason?: string): void {
        ...
        for (const session of mediaSessions) {
            ...
            remoteTracks &&
                (tracksToBeRemoved = [...tracksToBeRemoved, ...remoteTracks]);
    ...
    onIncomingCall(
        ...
        if (jingleSession.isP2P) {
            ...
            if (!this.isFocus(jingleSession.remoteJid)) {
                const description =
                    "Rejecting session-initiate from non-focus.";
    ...
    onCallEnded(
        ...
        logger.info(
            `Call ended: ${reasonCondition} - ${reasonText} P2P ?${jingleSession.isP2P}`
        );
        if (jingleSession === this.jvbJingleSession) {
            this.wasStopped = true;

            Statistics.sendAnalytics(
                createJingleEvent(AnalyticsEvents.ACTION_JINGLE_TERMINATE, {
                    p2p: false,
                })
            );
            ...
            if (
                reasonCondition === "connectivity-error" &&
    ...
    public getActivePeerConnection(): Nullable<TraceablePeerConnection> {
        const session = this.isP2PActive()
            ? this.p2pJingleSession
            : this.jvbJingleSession;
     ...
     * @returns {void}
     */
    public setStartMutedPolicy(policy: {
        audio: boolean;
        video: boolean;
    }): void {
        if (!this.isModerator()) {
            logger.warn(
                `Failed to set start muted policy, ${
        ...
        logger.info(
            `Setting start muted policy: ${JSON.stringify(
                policy
            )} in presence and in conference metadata`
        );
    ...
    public getSsrcByTrack(track: JitsiTrack): Optional<number> {
        return track.isLocal()
            ? this.getActivePeerConnection()?.getLocalSSRC(
        ...
        if (sendThroughVideobridge) {
            ...
            if (messageType === "object") {
                ...
                try {
                    ...
                    logger.error(
                        "Can not send a message, stringify failed: ",
                        e
                    );
    ...
    public isConnectionInterrupted(): boolean {
        return this.isP2PActive()
            ...
            : this.isJvbConnectionInterrupted;
    }
    ...
    public setDesktopSharingFrameRate(maxFps: number): boolean {
        if (!isValidNumber(maxFps)) {
            logger.error(
                `Invalid value ${maxFps} specified for desktop capture frame rate`
            );
     ...
     * @returns {void}
     */
    public setReceiverConstraints(
        videoConstraints: IReceiverVideoConstraints
    ): void {
        this.qualityController.receiveVideoController.setReceiverConstraints(
            videoConstraints
        );
    }
    ...
    public setAssumedBandwidthBps(assumedBandwidthBps: number): void {
        this.qualityController.receiveVideoController.setAssumedBandwidthBps(
            assumedBandwidthBps
        );
    }
    ...
    public setReceiverVideoConstraint(maxFrameHeight: number): void {
        this.qualityController.receiveVideoController.setPreferredReceiveMaxFrameHeight(
            maxFrameHeight
        );
    }
    ...
    public setSenderVideoConstraint(maxFrameHeight: number): Promise<void> {
        return this.qualityController.sendVideoController.setPreferredSendMaxFrameHeight(
            maxFrameHeight
        );
    }
     ...
     */
    public createVideoSIPGWSession(
        sipAddress: string,
        displayName: string
        ...
        return this.videoSIPGWHandler.createVideoSIPGWSession(
            sipAddress,
            displayName
        );
    }
    ...
    public toggleE2EE(enabled: boolean): void {
        if (!this.isE2EESupported()) {
            logger.warn(
                "Cannot enable / disable E2EE: platform is not supported."
            );
     ...
     * @returns {void}
     */
    public markParticipantVerified(
     ...
     */
    public addLobbyMessageListener(
        listener: (message: object) => void
    ): Optional<EventListener> {
        if (this.room) {
            return this.room
                .getLobby()
                .addMessageListener(listener) as Optional<EventListener>;
        }
    }
    ...
    public enableAVModeration(mediaType: MediaType): void {
        if (
            ...
            logger.warn(
                `Failed to enable AV moderation, ${
                    this.room ? "" : "not in a room, "
                }${
                    this.isModerator() ? "" : "participant is not a moderator, "
                }${
                    this.room && this.isModerator()
                        ? "wrong media type passed"
                        : ""
                }`
            );
        }
    ...
    public disableAVModeration(mediaType: MediaType): void {
        if (
            ...
            logger.warn(
                `Failed to disable AV moderation, ${
                    this.room ? "" : "not in a room, "
                }${
                    this.isModerator() ? "" : "participant is not a moderator, "
                }${
    ...
    public avModerationApprove(mediaType: MediaType, id: string): void {
        if (
            ...
            this.room
                .getAVModeration()
                .approve(mediaType, participant.getJid());
        } else {
            logger.warn(
                `AV moderation approve skipped , ${
                    this.room ? "" : "not in a room, "
                }${
                    this.isModerator() ? "" : "participant is not a moderator, "
                }${
